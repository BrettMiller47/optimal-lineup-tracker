...
export async function getDataByWeek(seasonId, leagueId, teams, weeksWithData) {
  ...
  try {
    ...
    // -------- headers[] -------- 
    // Navigate to team 1's first week score
    let headersUrl = `https://fantasy.espn.com/football/team?seasonId=${seasonId}&leagueId=${leagueId}&teamId=1&scoringPeriodId=1&statSplit=singleScoringPeriod`
    await driver.get(headersUrl);
    ...
    for (let week = 1; week < weeksWithData + 1; week++) {
      ...
      for (let team in teams) {
        ...
        // --------- populate players[{statDesc: stat, ...}] ---------
        let players = []
        ...
        weeklyData.push({ teamId: teamId, rawLineup: players });
      }
